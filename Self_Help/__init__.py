...
class Welcome(Page):
    form_model = 'player'
    form_fields = ['Sex', 'Age', 'Nation', 'Education', 'Law_Related', 'Marriage', 'Income']
    timeout_seconds = 120
...
class WaitForPlaintiff(WaitPage):
    ...
    body_text = '您被分配到的角色是：<b>侵权者</b>！目前被侵权者正在决策，请您稍作等待。'
    @staticmethod
    def is_displayed(player: Player):
        group = player.group
...
class WaitForAll(WaitPage):
    ...
    body_text = '实验已经接近尾声，请您稍作等待。'
class Result(Page):
    form_model = 'player'
    form_fields = ['Email']
class Thanks(Page):
    form_model = 'player'
page_sequence = [Wait, Welcome, Description, Plaintiff, WaitForPlaintiff, Defendant_1, Plaintiff_1, Defendant_2, Defendant_3, WaitForAll, Result, Thanks]